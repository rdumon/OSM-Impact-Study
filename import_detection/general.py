...
def analyse_import(db, iMport, x = None, y = None, city = '', dir_write_to =''):

	#===IMPORT===DATE===
	import_date = iMport[0][0]

	# ======= CREATE DIR FOR GRAPHS OF THIS SPECIFIC IMPORT======
	import_dir = dir_write_to + '/' +  import_date.strftime('%Y-%m-%d')
	if not os.path.exists(import_dir):
		os.makedirs(import_dir)


	# PRINT THE NODES OF THIS IMPORT
	draw_heatMap(db, iMport, x, y, city, import_dir)
